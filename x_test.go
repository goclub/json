...
func TestUnmarshalStringConvertIntEmptyString(t *testing.T) {
	autoNumberConvertString = true
	defer func() {
		autoNumberConvertString = false
	}()
	...
	err := Unmarshal([]byte(`{"Int":"","Int8":"","Int16":"","Int32":"","Int64":"","Uint":"","Uint8":"","Uint16":"","Uint32":"","Uint64":"","Float32": "", "Float64": ""}`), &query)
	assert.NoError(t, err)
	assert.Equal(t, 0, query.Int)
	assert.Equal(t, int8(0), query.Int8)
	assert.Equal(t, int16(0), query.Int16)
	assert.Equal(t, int32(0), query.Int32)
	assert.Equal(t, int64(0), query.Int64)
	assert.Equal(t, uint(0), query.Uint)
	assert.Equal(t, uint8(0), query.Uint8)
	assert.Equal(t, uint16(0), query.Uint16)
	assert.Equal(t, uint32(0), query.Uint32)
	assert.Equal(t, uint64(0), query.Uint64)
	assert.Equal(t, float32(0), query.Float32)
	assert.Equal(t, float64(0), query.Float64)
